...
def solve_rhomb():
    ...
    def rhomb_square(cur_points):
        for permut in permutations:
            p1, p2, p3, p4 = [cur_points[permut[i-1]-1] for i in range(4)]
            if p1 == p2 == p3 == p4:
            ...
            if scalar_product(v1, v2):
                # Диагонали должны быть перпендикулярны
                continue
    ...
    best_points = []
    best_square = 0
    combinations = comb(n, 4)  # все возможные комбинации 4х точек
    for c in combinations:  # перебираем текущую комбинацию
        cur_combination = [points[i-1] for i in c]  # точки текущей комбинации
        cur_square = rhomb_square(cur_combination)
        if cur_square > best_square:
